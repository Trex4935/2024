...
public class AlignWithPID extends Command {
    ...
    if (useVision) {
      ...
      if (useAllianceColor && DriverStation.getAlliance().get() == DriverStation.Alliance.Red) {
        Translation2d transformedTranslation =
            new Translation2d(pose.getX(), Alignment.fieldWidth - pose.getY());
  ...
  public void execute() {
    if (useVision) {
      if (!LimelightHelpers.getTV("limelight-testll")) {
        this.cancel();
      }
    }
  ...
  public void end(boolean interrupted) {
    ...
    finalDrive =
        finalDrive
            .withSpeeds(new ChassisSpeeds(emptyTranslation.getX(), emptyTranslation.getY(), 0.0))
